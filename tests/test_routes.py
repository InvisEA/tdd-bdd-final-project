...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        """It should get a product with a required ID"""
    ...
    def test_update_a_product(self):
        ...
        self.assertEqual(data["name"], test_product.name)
        prod_id = data["id"]
        data["name"] = "SUPER UPDATE"
        response = self.client.put(f"{BASE_URL}/{prod_id}", json=data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
    ...
    def test_query_by_availability(self):
        ...
        products = self._create_products(10)
        available_prods = [product for product in products if product.available is True]
        avail_count = len(available_prods)
        logging.debug("Found Products [%d] %s", avail_count, available_prods)
